            ...
            return Manager.Database.DoActionAsync(db =>
            {
                try
                {
                    ...
                    if (Character != null)
                    {
                        ...
                        var questListTemp = new List<Tuple<int, int, bool, bool>>();
                        foreach (var i in Player.playerQuestManager.GetPlayerQuests())
                        {
                            questListTemp.Add(new Tuple<int, int, bool, bool>(i.id,i.actualProgress,i.completed,i.rewarded));
                        }
                        db.CharacterQuestSave(Character.CharacterId, questListTemp);
                    }
