...
namespace wServer.realm
{
    public class ChatManager
    {
        ...
        public void Say(Player src, string text)
        {
            ...
            {
                Name = (src.Client.Account.Rank >= 11 ? "@" : src.Client.Account.Rank >= 10 ? "#[Owner] " : src.Client.Account.Rank >= 9 ? "#[Co Owner] " : src.Client.Account.Rank >= 8 ? "#[Head Developer] " : src.Client.Account.Rank >= 7 ? "#[Developer] " : src.Client.Account.Rank >= 6 ? "#[Admin] " : src.Client.Account.Rank >= 5 ? "#[Moderator] " : src.Client.Account.Rank >= 4 ? "#[Helper] " : src.Client.Account.Rank >= 3 ? "#[VIP] " : src.Client.Account.Rank >= 2 ? "#[Supporter] " : src.Client.Account.Rank >= 1 ? "#[Donor] " : src.Client.Account.Rank >= 0 ? "[Player] " : "") + src.Name,
                ObjectId = src.Id,
                Stars = src.Stars,
                BubbleTime = 10,
                Recipient = "",
